...
export default function CartDropdown({ onClose }: CartDropdownProps) {
  ...
  return (
    <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-background border rounded-lg shadow-lg z-50 p-4 max-h-[80vh] overflow-auto">
            ...
            {cartItems.map((item) => (
              <div key={`${item.id}-${item.size}`} className="flex items-center gap-4 border-b pb-4">
                <div className="relative h-16 w-16 bg-muted rounded">
                  <img
                    src={item.image || "./../assets/other-images/placeholder.webp"}
                    alt={item.name}
